package net.minecraft.entity.player;

/**
 * B3 compile stub: shape-only vanilla player type used as a constructor
 * argument by the dev-only autoplay spike proof (tools/autoplay/, never
 * shipped), which posts a simulated harvest with a null player (the
 * bridge hook never reads it). Never runs (compile classpath only). Used
 * as a type only — no member is called, so no pin is owned here (the
 * {@code BreakEvent(} constructor is pinned by
 * tools/autoplay/universal-pin.txt).
 */
public class EntityPlayer {
}
