#!/bin/sh
# B3 live gate: Forge 1.7.10-1614 server run proving PackWire.bind (real
# Block resolve) plus world-tick apply on a real world, then comparing the
# world against the pure decision union (tools/live).
#
# Manual gate (needs network + Java 8 + 1614 SRG mappings); NOT part of
# tools/check.sh. Never silent: any mismatch fails loudly, never defaulted.
#
...
#   SRG_MCP    path to srg-mcp.srg for 1.7.10-1614 (REQUIRED, ForgeGradle
#              cache or userdev; pins stubs and drives reobf)
...
[ -n "${SRG_MCP:-}" ] || { echo "FAIL b3-live : SRG_MCP env required (srg-mcp.srg for 1.7.10-1614)"; exit 1; }
[ -f "$SRG_MCP" ] || { echo "FAIL b3-live : SRG_MCP=<$SRG_MCP> missing"; exit 1; }
J8="$JAVA8_HOME/bin"
...
# 2. Provision the 1614 server once (idempotent).
mkdir -p "$B3_DIR"
SERV="$B3_DIR/server"
mkdir -p "$SERV"
...
ASM=$(find "$SERV/libraries/org/ow2/asm" -name "asm-all-*.jar" | head -n 1)
[ -n "$ASM" ] || { echo "FAIL b3-live : ASM missing from server libs"; exit 1; }
echo "ok b3-live : server provisioned"
